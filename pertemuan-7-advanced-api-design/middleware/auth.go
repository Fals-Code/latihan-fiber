...
func RequireAuth(jwtManager *helper.JWTManager) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			return helper.NewAppError(fiber.StatusUnauthorized, "header Authorization tidak ada atau salah bentuk")
		}
		identity, err := jwtManager.ParseAccessToken(raw)
		if errors.Is(err, helper.ErrExpiredToken) {
			return helper.NewAppError(fiber.StatusUnauthorized, "access token kedaluwarsa")
		}
		if err != nil {
			return helper.NewAppError(fiber.StatusUnauthorized, "access token tidak valid")
		}
...
func LoginRateLimiter() fiber.Handler {
	return limiter.New(limiter.Config{
		...
		LimitReached: func(c *fiber.Ctx) error {
			...
			return helper.NewAppError(fiber.StatusTooManyRequests, "terlalu banyak percobaan login, coba lagi dalam satu menit")
		},
	})
}
