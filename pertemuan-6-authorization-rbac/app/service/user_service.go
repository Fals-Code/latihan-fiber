...
import (
	...
	"tugas1-go/pertemuan-6-authorization-rbac/app/repository"
...
func (s *UserService) Get(c *fiber.Ctx) error {
	...
	if !CanAccessUser(current, id, s.perms, "user:read:any") {
		return helper.Fail(c, fiber.StatusForbidden, "tidak memiliki akses ke pengguna ini")
	}
	ctx, cancel := helper.RequestContext(c)
	defer cancel()
...
func (s *UserService) Create(c *fiber.Ctx) error {
	...
	if req.Username == "" || req.Email == "" || req.Password == "" {
		return helper.Fail(c, fiber.StatusUnprocessableEntity, "username, email, dan password wajib diisi")
	}
	...
	if !s.perms.IsKnownRole(role) {
		return helper.Fail(c, fiber.StatusUnprocessableEntity, "role tidak dikenal")
	}
...
func (s *UserService) update(c *fiber.Ctx) error {
	...
	if !CanAccessUser(current, id, s.perms, "user:update:any") {
		return helper.Fail(c, fiber.StatusForbidden, "tidak memiliki akses untuk mengubah pengguna ini")
	}
