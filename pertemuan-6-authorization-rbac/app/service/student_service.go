...
func (s *StudentService) Get(c *fiber.Ctx) error {
	...
	if !canAccessStudentOwner(current, ownerID, s.perms, "student:read:any") {
		return helper.Fail(c, fiber.StatusForbidden, "tidak memiliki akses ke mahasiswa ini")
	}
...
func (s *StudentService) Replace(c *fiber.Ctx) error {
	...
	if !canAccessStudentOwner(current, ownerID, s.perms, "student:update:any") {
		return helper.Fail(c, fiber.StatusForbidden, "tidak memiliki akses untuk mengubah mahasiswa ini")
	}
	student, err := s.repo.FindByID(ctx, id)
	if err != nil {
		return translateRepositoryError(c, err)
